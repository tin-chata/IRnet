...
def process_schema(schema):
    new_schema = defaultdict()
    for tbk, tbv in schema.items():
        tb_id, cols = tbv
        new_cols = {}
        for colk, colv in cols.items():
            new_colv = [wd.lower() for wd in colv]
            new_cols[colk.lower()] = new_colv
        new_tbv = [tb_id.lower(), new_cols]
        new_schema[tbk.lower()] = new_tbv
    return new_schema


def main(sample, tables):
    question = sample["question"]
    query = sample["query"]
    schema = tables[sample["db_id"]][-1]
    # table_names = list(schema.keys())
    # table_name = table_names[0]
    # column_names = list(schema[table_name][-1].keys())
    print(sample["db_id"])
    print(question)
    print(query)
    ...
    try:
        ...
        parsed_sql = parser.get_sql(query)
        print(parsed_sql, '\n')
        return question, query, parsed_sql, parser
    except:
        print(question)
        print(query)
        print("Error in db_id: ", sample["db_id"])
...
class csv2json(object):
    def __init__(self, labeled_csv=str(), output_json_file=str(), TablesJSONPath=str(), DB_Name=str()):
        self.training_list_global = list()
        self.db_id = DB_Name
        self.table_file = TablesJSONPath  # JHV changed this file to be the one that I will specify  "tables.json"
        self.labeled_csv = labeled_csv
        self.output_json_file = output_json_file
        self.schemas, self.db_names, self.tables = get_schemas_from_json(self.table_file)
        self.sql_parser = sqlParser(self.schemas[self.db_id], self.tables[self.db_id])

    @staticmethod
    def generate_sql_tokens(the_sql_query):
        sql_query_list = the_sql_query.split()
        sql_query = ' '.join(sql_query_list)
        i = 0
        while i < len(sql_query_list):
            current_token = sql_query_list[i]
            last_split_j = 0
            for j in range(len(current_token)):
                if current_token[j] == '(':
                    sql_query_list[i] = current_token[:j]
                    i += 1
                    sql_query_list.insert(i, '(')
                    last_split_j = j
                elif current_token[j] == ')':
                    i += 1
                    sql_query_list.insert(i, current_token[last_split_j + 1:j])
                    i += 1
                    sql_query_list.insert(i, ')')
            i += 1
        return sql_query, sql_query_list

    @staticmethod
    def generate_sql_query_list_no_value(sql_query_list):
        sql_query_list_no_value = copy.copy(sql_query_list)
        for i in range(len(sql_query_list)):
            current_token = sql_query_list_no_value[i].lower()
            try:
                x = float(current_token)
                current_token = 'value'
            except:
                try:
                    x = int(current_token)
            ...
            if current_token[0] == "'":
                current_token = 'value'
            sql_query_list_no_value[i] = current_token

        return sql_query_list_no_value

    def process_CSV(self):
        i = 0
        df = pd.read_csv(self.labeled_csv)
        ...
        for index, row in df.iterrows():
            ...
            print(eng_query)
            print(sql_query)
            label_dict['db_id'] = self.db_id
            label_dict_query, label_dict_query_toks = csv2json.generate_sql_tokens(sql_query)
            label_dict['query'] = label_dict_query
            label_dict['query_toks'] = label_dict_query_toks
            label_dict['query_toks_no_value'] = csv2json.generate_sql_query_list_no_value(label_dict_query_toks)
            print(label_dict['query_toks_no_value'])
            label_dict['question'] = eng_query
            label_dict['question_toks'] = eng_query.split()
            sql_label = self.sql_parser.get_sql(sql_query)
            label_dict['sql'] = sql_label
            self.training_list_global.append(label_dict)
            i += 1

    def write_to_json_file(self):
        if not os.path.exists(os.path.dirname(self.output_json_file)):
            os.mkdir(os.path.dirname(self.output_json_file))

        with open(self.output_json_file, 'w') as outfile:
            json.dump(self.training_list_global, outfile, indent=2)
...
if __name__ == '__main__':
    ...
    """
    from np5.utils.sql2sem import JSON, SCHEMA

    data = JSON.load("../../data/nl2sql/train.json")
    database = SCHEMA("../../data/nl2sql/tables.json")
    # schemas, db_names, tables = get_schemas_from_json("../../data/nl2sql/tables.json")

    for d in data:
        db_id = d['db_id']
        tbcoldict, tbcolnames = database.get_tbcoldict(db_id)
        sql_parser = sqlParser(tbcoldict, tbcolnames)
        try:
            sql_tree = sql_parser.get_sql(d["query"])
            d["sql_tree"] = sql_tree
        except:
            print(d["question"])
            print(d["query"])
