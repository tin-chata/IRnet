...
class POSTPROCESS:
    ...
    def find_table(cur_table, origin_table_names, question_arg_type, question_arg):
        # h_table = None
        # Find fully matched table which differs from the current table is mentioned in the question
        ...
        # if h_table != cur_table:
        #     return h_table
        # find partially matched table which differs from the current table is mentioned in the question
        for i in range(len(question_arg_type))[::-1]:
            if question_arg_type[i] == ['NONE']:
                for t_id, table_name in enumerate(origin_table_names):
                    if POSTPROCESS.partial_match(question_arg[i], table_name) and t_id != cur_table:
                        return t_id

        # randomly return a table that is mentioned in the question
        for i in range(len(question_arg_type))[::-1]:
            if question_arg_type[i] == ['table']:
                h_table = question_arg[i]
                h_table = origin_table_names.index(h_table)
                return h_table
        # otherwise return a current table
        return cur_table
    ...
    def alter_not_in(d, schema):
        ...
        if 'Filter(19)' in d['model_result']:
            current_table = copy.deepcopy(schema)
            ...
            origin_table_names = [[wordnet_lemmatizer.lemmatize(x.lower())
                                   for x in names.split(' ')] for names in current_table['table_names']]
            ...
            for label_id, label_val in enumerate(pred_label):
                if label_val in ['Filter(19)']:
                    ...
                    break
            # Find mentioned table id in NL question
            ...
            for label_id, label_val in enumerate(pred_label):
                if label_val in ['Filter(19)']:
                    for primary in current_table['primary_keys']:
                        # check if the primary key is using in the mentioned table appeared before 'Filter(19)'
                        if int(current_table['col_table'][primary]) == int(pred_label[label_id - 1][2:-1]):
                            col_id = current_table['col_set'].index(current_table['schema_content_clean'][primary])
                            # Re-label the column appeared after 'Filter(19)' to the primary key
                            pred_label[label_id + 2] = 'C(' + str(col_id) + ')'
                            break
                    for pair in current_table['foreign_keys']:
                        # foreign key which borrows from the primary key
                        col_id0 = current_table['col_set'].index(current_table['schema_content_clean'][pair[0]])
                        # primary key
                        col_id1 = current_table['col_set'].index(current_table['schema_content_clean'][pair[1]])
                        if int(current_table['col_table'][pair[0]]) == h_table and \
                                col_id1 == int(pred_label[label_id + 2][2:-1]):
                            pred_label[label_id + 8] = 'C(' + str(col_id0) + ')'
                            pred_label[label_id + 9] = 'T(' + str(h_table) + ')'
                            break
                        elif int(current_table['col_table'][pair[1]]) == h_table and \
                                col_id0 == int(pred_label[label_id + 2][2:-1]):
                            pred_label[label_id + 8] = 'C(' + str(col_id1) + ')'
                            pred_label[label_id + 9] = 'T(' + str(h_table) + ')'
                            break
    ...
    def alter_inter(d):
        if 'Filter(0)' in d['model_result']:
            ...
            replace_result = replace_result + \
                             now_result[now_result.index('Filter(0)') + 1: r_id + now_result.index('Filter(0)') + 2]
    ...
    def alter_column0(d, schema):
        ...
        if 'C(0)' in d['model_result']:
            ...
            origin_table_names = [[wordnet_lemmatizer.lemmatize(x.lower())
                                   for x in names.split(' ')] for names in schema['table_names']]
            count += 1
            easy_flag = False
